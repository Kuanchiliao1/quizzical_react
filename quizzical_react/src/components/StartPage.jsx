...
export default function StartPage(props) {
  ...
  return (
    props.viewSaved ? <SavedQuestionsPage storedQuizData={props.storedQuizData} setStoredQuizData={props.setStoredQuizData} handleViewSavedBtn={handleViewSavedBtn}/> :
    <div className="start-page">
      ...
      <div className="quiz-components-container">
        ...
        <button className="start-quiz" onClick={props.start}>Start quiz!</button>
        <p className="score-message">{getScoreMessage()}</p>
        <div className="reset-save-btn-container">
          <button disabled={isReset || getScoreMessage() === "No score yet"} className="reset-score" onClick={handleResetBtn}>Reset</button>
          <button className="view-saved-questions" onClick={handleViewSavedBtn}>⭐ Saved</button>
        </div>
      </div>
    </div>
  )
